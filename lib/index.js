...
function compileSass(options) {
    ...
    var sourcemap = (options.sourcemap === false) ? false : true;
    var sass;
    ...
    return function (req, res, next) {
        ...
        function sassError(err) {
            // node-sass 2.x sends a nice error object
            if (typeof err === 'object' && err.file && err.line && err.column) {
                sendErrorResponse('express-compile-sass:\n  Syntax error in ' + req.originalUrl + ' line ' + err.line + ':' + err.column + '\n' + err.message);
            } else {
            ...
            if (req.fresh) {
                ...
                sass.render({
                    ...
                    success: function (result) {
                        ...
                        var etag = crypto.createHash('md5').update(result.css).digest('hex').substr(0, 16) + '-compile-sass';
