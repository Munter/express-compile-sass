...
function compileSass(options) {
  ...
  function watchImports(main, imports) {
    ...
    try {
      if (!fileWatcher) {
        ...
        fileWatcher.on('change', fileChanged).on('all', function(event, path) {
  ...
  function middleware(ctx, next) {
    function sendErrorResponse(err) {
      ...
      log(err);
      ctx.body = csserror(err);
      ctx.status = 200;
    }

    function sassError(err) {
      var errStr = 'express-compile-sass:\n  Syntax error in ' + ctx.originalUrl + ':' + err.line;
    ...
    if (/\.(?:scss|sass)$/.test(ctx.path)) {
      if (!sass) {
        try {
          sass = require('node-sass');
        } catch (e) {
          return sendErrorResponse(
            'express-compile-sass: Unable to load the node-sass module. Please run `npm install --save node-sass`'
          );
        }
      }

      var fileUrl = Path.join(Path.resolve(options.root), ctx.path);
      ctx.set({
      ...
      if (ctx.fresh) {
        ...
        ctx.body = cache[fileUrl];
        ctx.status = etag.replace(/\"/g, '') === etagmap[fileUrl] ? 304 : 200;
        return;
      } else {
        // Compile sass
